...
/*   Updated: 2025/02/26 20:18:19 by gavivas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (1) 
	{
		...
		if (!data) //si data no existe la crea vacia
			data = ft_strdup("");
		...
		free(data); //libera lo que data tenia guardado.
	...
	if (data && *data) //si tada todavia tiene algo guardado
	{
		if (!ft_strchr(data, '\n'))
			...
			newline = ft_strdup(data); //si es el fin de el archivo retorna hasta el null
		free(data); 
		data = NULL;
		return (newline);
	}
